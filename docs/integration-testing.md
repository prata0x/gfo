...
3. Repository access: **Only select repositories** → `gfo-integration-test` を選択
4. Permissions:
...
#### API トークン（App Password）取得

Bitbucket は Personal Access Token ではなく **App Password** を使用する。

1. https://bitbucket.org/account/settings/app-passwords/ を開く
...
4. Permissions:
   ...
   - Issues: **Read**, **Write**
5. Create してパスワードをコピー

#### 環境変数

トークンの形式は **`username:app-password`**（コロン区切り）。

```bash
GFO_TEST_BITBUCKET_APP_PASSWORD=your-username:ATBB-xxxxxxxxxxxx
...
- トークンのスコープ・権限を再確認する
- Bitbucket は `username:app-password` 形式になっているか確認する
