...
|---|---|---|---|---|---|
| リポジトリ指定 | `--repo` / `-R` (`URL` or `HOST/OWNER/REPO`) | `--repo` / `-R` (`OWNER/REPO`) | `--repo` / `-R` (`OWNER/REPO`) | `--repo` / `-r` | `--repo` / `-r` (サブコマンドごと) |
...
| ページネーション | `--limit` / `-L` (サブコマンドごと) | `--limit` / `-L` | `--per-page` / `-P`, `--page` / `-p` | `--limit` / `-l`, `--page` / `-p` | `--page` / `-p` (一部コマンド) |
...
gfo の 35 トップレベルコマンドを基準に、各ツールの対応コマンド名を一覧する。
...
| revert | `---` | `pr revert` | `---` | `---` | `---` |
| status | `pr status` | `pr status` | `---` | `---` | `pr status` (`--wait`) |
| browse | `---` | `---` | `---` | `---` | `pr browse` |
...
| オプション | gfo | gh | glab | tea | fj |
|---|---|---|---|---|---|
| 状態 | `--state` / `-s` | `--state` / `-s` | `--closed` / `-c`, `--merged` / `-M`, `--all` / `-A` | `--state` | `--state` / `-s` |
| 件数上限 | `--limit` / `-L` | `--limit` / `-L` | `--per-page` / `-P`, `--page` / `-p` | `--limit` / `--page` | `---` |
| 作者 | `--author` / `-A` | `--author` / `-A` | `--author` | `--author` / `-A` | `--creator` / `-c` |
| 担当者 | `--assignee` / `-a` | `--assignee` / `-a` | `--assignee` / `-a` | `--assignee` / `-a` | `--assignee` / `-a` |
...
| マイルストーン | `--milestone` / `-m` | `---` (`--search` 経由) | `--milestone` / `-m` | `--milestones` / `-m` | `---` |
| 検索クエリ | `--search` / `-S` | `--search` / `-S` | `--search` | `--keyword` / `-k` | 位置引数 (query) |
| ドラフト | `--draft` / `-d` / `--no-draft` | `--draft` / `-d` | `--draft` / `-d`, `--not-draft` | `---` | `---` |
...
| オプション | gfo | gh | glab | tea | fj |
|---|---|---|---|---|---|
| タイトル | `--title` / `-t` | `--title` / `-t` | `--title` / `-t` | `--title` / `-t` | 位置引数 (`WIP:` 接頭辞でドラフト) |
| 本文 | `--body` / `-b` / `--body-file` / `-F` | `--body` / `-b` / `--body-file` / `-F` | `--description` / `-d` | `--description` / `-d` | `--body` / `--body-file` |
| ベースブランチ | `--base` / `-B` | `--base` / `-B` | `--target-branch` / `-b` | `--base` / `-b` | `--base` |
| ヘッドブランチ | `--head` / `-H` | `--head` / `-H` | `--source-branch` / `-s` | `--head` | `--head` |
| ドラフト | `--draft` / `-d` | `--draft` / `-d` | `--draft` | `---` | `WIP:` 接頭辞 (タイトル) |
| レビュアー | `--reviewer` / `-r` | `--reviewer` / `-r` | `--reviewer` | `---` | `---` |
| 担当者 | `--assignee` / `-a` | `--assignee` / `-a` | `--assignee` / `-a` | `--assignees` / `-a` | `---` |
| ラベル | `--label` / `-l` | `--label` / `-l` | `--label` / `-l` | `--labels` / `-L` | `---` |
| マイルストーン | `--milestone` / `-m` | `--milestone` / `-m` | `--milestone` / `-m` | `--milestone` / `-m` | `---` |
| 期限 | `---` | `---` | `---` | `--deadline` / `-D` | `---` |
| 自動入力 | `--fill` / `-f` | `--fill` / `-f` | `--fill` / `-f` | `---` | `--autofill` / `-A` |
| プロジェクト | `---` | `--project` / `-p` | `---` | `---` | `---` |
| Web で作成 | `--web` / `-w` | `--web` / `-w` | `--web` / `-w` | `---` | `--web` / `-w` |
| ソースブランチ削除 | `---` | `---` | `--remove-source-branch` | `---` | `---` |
| スカッシュ | `---` | `---` | `--squash-before-merge` | `---` | `---` |
| ドライラン | `--dry-run` | `--dry-run` | `---` | `---` | `---` |
| AGit ワークフロー | `---` | `---` | `---` | `---` | `--agit` |
...
| オプション | gfo | gh | glab | tea | fj |
|---|---|---|---|---|---|
| マージコミット | `--merge` / `-m` | `--merge` / `-m` | `---` (デフォルト) | `--style merge` / `-s` | `--method merge` / `-M` |
| スカッシュ | `--squash` / `-s` | `--squash` / `-s` | `--squash` / `-s` | `--style squash` / `-s` | `--method squash` / `-M` |
| リベース | `--rebase` / `-r` | `--rebase` / `-r` | `--rebase` / `-r` | `--style rebase` / `-s` | `--method rebase` / `-M` |
...
| オプション | gfo | gh | glab | tea | fj |
|---|---|---|---|---|---|
| タイトル | `--title` / `-t` | `--title` / `-t` | `--title` / `-t` | `--title` / `-t` | `pr edit <id> title` |
| 本文 | `--body` / `-b` | `--body` / `-b` | `--description` / `-d` | `--description` / `-d` | `pr edit <id> body` |
| ベースブランチ | `--base` / `-B` | `--base` / `-B` | `--target-branch` | `---` | `---` |
| ラベル追加/削除 | `--add-label` / `--remove-label` | `--add-label` / `--remove-label` | `--label` / `-l` / `--unlabel` / `-u` | `--add-labels` / `-L` / `--remove-labels` | `pr edit <id> labels --add` / `--rm` |
...
| マイルストーン | `--milestone` / `-m` | `--milestone` / `-m` / `--remove-milestone` | `--milestone` / `-m` | `--milestone` / `-m` | `---` |
| ドラフト切替 | `--draft` / `-d` / `--ready` | `---` | `--draft` / `--ready` | `---` | `---` |
...
| subscribe | `issue subscribe` / `unsubscribe` | `---` | `issue subscribe` / `unsubscribe` | `---` | `---` |
...
| オプション | gfo | gh | glab | tea | fj |
|---|---|---|---|---|---|
| 状態 | `--state` / `-s` | `--state` / `-s` | `--state` | `--state` | `--state` / `-s` |
| 担当者 | `--assignee` / `-a` | `--assignee` / `-a` | `--assignee` / `-a` | `--assigned-to` / `-a` | `--assignee` / `-a` |
| ラベル | `--label` / `-l` | `--label` / `-l` | `--label` / `-l` | `--labels` / `-L` | `--labels` / `-l` |
| 作者 | `--author` / `-A` | `--author` / `-A` | `--author` | `--author` / `-A` | `--creator` / `-c` |
| マイルストーン | `--milestone` / `-m` | `--milestone` / `-m` | `--milestone` / `-m` | `--milestone` | `---` |
| 件数上限 | `--limit` / `-L` | `--limit` / `-L` | `--per-page` / `-P`, `--page` / `-p` | `--limit` | `---` |
| 検索クエリ | `--search` / `-S` | `--search` / `-S` | `--search` / `--in` | `--keyword` / `-k` | 位置引数 (query) |
| 機密 | `---` | `---` | `--confidential` / `-C` | `---` | `---` |
...
| オプション | gfo | gh | glab | tea | fj |
|---|---|---|---|---|---|
| タイトル | `--title` / `-t` | `--title` / `-t` | `--title` / `-t` | `--title` / `-t` | 位置引数 |
| 本文 | `--body` / `-b` / `--body-file` / `-F` | `--body` / `-b` / `--body-file` / `-F` | `--description` / `-d` | `--description` / `-d` | `--body` / `--body-file` |
| 担当者 | `--assignee` / `-a` | `--assignee` / `-a` | `--assignee` / `-a` | `--assignees` / `-a` | `---` |
| ラベル | `--label` / `-l` | `--label` / `-l` | `--label` / `-l` | `--labels` / `-L` | `---` |
| マイルストーン | `--milestone` / `-m` | `--milestone` / `-m` | `--milestone` / `-m` | `--milestone` / `-m` | `---` |
...
| オプション | gfo | gh | glab | tea | fj |
|---|---|---|---|---|---|
| タイトル | `--title` / `-t` | `--title` / `-t` | `--title` / `-t` | `--title` / `-t` | `issue edit <id> title` |
| 本文 | `--body` / `-b` | `--body` / `-b` | `--description` / `-d` | `--description` / `-d` | `issue edit <id> body` |
| 担当者 | `--assignee` / `-a` / `--add-assignee` / `--remove-assignee` | `--add-assignee` / `--remove-assignee` | `--assignee` / `-a` / `--unassign` | `--add-assignees` / `-a` | `---` |
| ラベル | `--label` / `-l` / `--add-label` / `--remove-label` | `--add-label` / `--remove-label` | `--label` / `-l` / `--unlabel` / `-u` | `--add-labels` / `-L` / `--remove-labels` | `issue edit <id> labels --add` / `--rm` |
| マイルストーン | `--milestone` / `-m` | `--milestone` / `-m` / `--remove-milestone` | `--milestone` / `-m` | `--milestone` / `-m` | `---` |
...
| 内部 | `---` | `---` | `--internal` | `---` | `---` |
| 説明 | `--description` / `-d` | `--description` / `-d` | `--description` / `-d` | `--description` | `--description` / `-d` |
...
| オプション | gfo | gh | glab | tea | fj |
|---|---|---|---|---|---|
| リネーム | `--name` | `repo rename` (別コマンド) | `---` | `---` | `---` |
| 説明 | `--description` / `-d` | `--description` | `--description` | `---` | `---` |
...
| タイトル | `--title` / `-t` | `--title` / `-t` | `--name` / `-n` | `--title` / `-t` | 位置引数 (name) |
| ノート | `--notes` / `-n` | `--notes` / `-n` | `--notes` / `-N` | `--note` / `-n` | `--body` / `-b` |
| ドラフト | `--draft` / `-d` | `--draft` / `-d` | `---` | `--draft` / `-d` | `--draft` / `-d` |
| プレリリース | `--prerelease` / `-p` | `--prerelease` / `-p` | `---` | `--prerelease` / `-p` | `--prerelease` / `-p` |
| ターゲット ref | `--target` | `--target` | `--ref` / `-r` | `--target` | `--branch` / `-B` |
| マイルストーン | `---` | `---` | `--milestone` / `-m` | `---` | `---` |
| ノートファイル | `--notes-file` / `-F` | `--notes-file` / `-F` | `--notes-file` / `-F` | `--note-file` / `-f` | `---` |
...
| オプション | gfo | gh | glab | tea | fj |
|---|---|---|---|---|---|
| タイトル | `--title` / `-t` | `--title` / `-t` | `---` | `--title` / `-t` | `--rename` / `-n` |
...
|---|---|---|---|---|---|
| 名前 | 位置引数 | 位置引数 | `--name` / `-n` | 位置引数 | 位置引数 |
| 色 | `--color` / `-c` (RRGGBB) | `--color` / `-c` | `--color` / `-c` | `--color` | 位置引数 (hex) |
| 説明 | `--description` / `-d` | `--description` / `-d` | `--description` / `-d` | `--description` | `--description` / `-d` |
| 優先度 | `---` | `---` | `--priority` / `-p` | `---` | `---` |
| 排他ラベル | `---` | `---` | `---` | `---` | `--exclusive` / `-x` |
| アーカイブ | `---` | `---` | `---` | `---` | `--archived` / `-a` |
...
| オプション | gfo | gh | glab | tea | fj |
|---|---|---|---|---|---|
| リネーム | `--name` / `-n` | `--name` / `-n` | `--new-name` / `-n` | `--name` | `--name` / `-n` |
| 色 | `--color` / `-c` | `--color` / `-c` | `--color` / `-c` | `--color` | `--color` / `-c` |
| 説明 | `--description` / `-d` | `--description` / `-d` | `--description` / `-d` | `--description` | `--description` / `-d` |
| 優先度 | `---` | `---` | `--priority` / `-p` | `---` | `---` |
| 排他ラベル | `---` | `---` | `---` | `---` | `--exclusive` / `-x` |
| アーカイブ | `---` | `---` | `---` | `---` | `--archived` / `-a` |
...
| view | `tag view` | `---` | `---` | `---` | `tag view` |
| delete | `tag delete` | `---` | `---` | `---` | `tag delete` |

> gfo の `tag create` は `--ref` でベースを、`--message` / `-m` でアノテーションを指定。
...
| artifact list | `ci artifact list` | `---` | `---` | `---` | `---` |
| artifact download | `ci artifact download` | `run download` | `---` | `---` | `---` |
| run (新規作成) | `---` | `---` | `ci run` | `---` | `---` |
| lint | `---` | `---` | `ci lint` | `---` | `---` |
| status | `---` | `---` | `ci status` | `---` | `---` |
| get | `---` | `---` | `ci get` | `---` | `---` |
...
| view | `ssh-key view` | `---` | `---` | `---` | `user key view` |
...
| view | `gpg-key view` | `---` | `---` | `---` | `user gpg view` |
...
| edit | `webhook edit` | `---` | `---` | `webhooks update` | `---` |
...
| view | `deploy-key view` | `---` | `deploy-key get` | `---` | `---` |
