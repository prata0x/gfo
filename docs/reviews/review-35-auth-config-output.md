...
| ID | 重大度 | ファイル | 概要 | 状態 |
|----|--------|---------|------|------|
| **R35-01** | 🟡 中 | `auth.py` L116-123 | `load_tokens()` の `OSError` 未捕捉 | ✅ 修正済み |
| **R35-02** | 🟡 中 | `config.py` L242 | エラーメッセージの引数名 `project_key` が誤り | ✅ 修正済み |
| **R35-03** | 🟡 中 | `output.py` L71 | `None` フィールドが table で `"None"` と出力 | ✅ 修正済み |
| **R35-04** | 🟢 軽微 | テスト各種 | R35-01〜03 の修正確認テストなし | ✅ 修正済み |
...
| コミット | 修正内容 |
|---------|---------|
| （次のコミット） | R35-01〜04 — auth/config/output 修正・テスト追加 |
