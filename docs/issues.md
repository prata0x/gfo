...
### 対応サービス数 3 以上（gfo 閾値を超える）

| 機能 | 対応数 | 難易度 | 備考 |
|---|---|---|---|
| `runner`（CI Runner 管理）| 6 | B | サービス固有度が高く、階層構造・スコープ・権限モデルが異なる。unsupported.md に除外理由あり |
| `issue board` | 5 | B | ボード定義がサービスごとに全く異なる。unsupported.md に除外理由あり |

### 対応サービス数 2（閾値未満だが将来検討）

| 機能 | 対応サービス | 難易度 | 備考 |
|---|---|---|---|
| `ci schedule` | GitLab, Bitbucket | A | 他サービスは YAML 定義で API 管理不可 |
| `ci lint` | GitLab, Azure DevOps | A | CI 設定形式がサービスごとに異なる |
| `cache`（CI キャッシュ）| GitHub, Bitbucket | A | 対応が限定的 |
| `pr list --reviewer` | GitLab, Azure DevOps | S | GitHub は Search API 経由で間接的に可能だが方式が異なる |

### 対応サービス数 1（実装対象外）

`unsupported.md` に記載済み。`pr revert`, `pr delete`, `pr todo`, `repo autolink` 等。
