...
### Batch 10: CRUD 一貫性修正（view 欠落）

**変更ファイル**: `adapter/base.py`, `adapter/*.py`, `commands/repo.py`, `cli.py`

| # | 課題 | API 対応サービス |
|---|---|---|
| F1-1 | `branch view` | GitHub/GitLab/Bitbucket/Azure DevOps/Gitea/Forgejo/Gogs/GitBucket (8) |
| F1-2 | `tag view` | GitHub/GitLab/Gitea/Forgejo (4) |
| F1-3 | `deploy-key view` | GitHub/GitLab/Bitbucket/Gitea/Forgejo/Gogs (6) |
| F1-4 | `ssh-key view` | GitHub/GitLab/Bitbucket/Gitea/Forgejo/Gogs (6) |
| F1-5 | `gpg-key view` | GitHub/GitLab/Bitbucket/Gitea/Forgejo (5) |

list/create/delete がある各コマンドに `view` を追加。基底クラスに `get_branch()` / `get_tag()` / `get_deploy_key()` / `get_ssh_key()` / `get_gpg_key()` メソッドを追加。

---

### Batch 11: CRUD 一貫性修正（edit 欠落）

**変更ファイル**: `adapter/base.py`, `adapter/*.py`, `commands/webhook.py`, `commands/repo.py`, `commands/release.py`, `cli.py`

| # | 課題 | API 対応サービス |
|---|---|---|
| F2-1 | `webhook edit` | GitHub/GitLab/Bitbucket/Backlog/Gitea/Forgejo/Gogs/GitBucket (8) |
| F2-2 | `org edit` | GitHub/GitLab/Gitea/Forgejo/Gogs (5) |
| F2-3 | `release asset edit` | GitHub/GitLab/Gitea/Forgejo (4) |
| F2-4 | `tag-protect edit` | GitLab/Gitea/Forgejo (3) |

create/delete があるのに edit がないコマンドに edit を追加。F2-1 は `update_webhook()` メソッド追加（URL・イベント・シークレット変更）。F2-2 は `update_organization()` メソッド追加（説明・表示名変更）。F2-3 は `update_release_asset()` メソッド追加（アセット名変更）。F2-4 は `update_tag_protection()` メソッド追加。
...
### F1. list/create/delete はあるが view がない

| # | コマンド | 現状 | API 対応サービス | 備考 |
|---|---|---|---|---|
| F1-1 | `branch view` | list/create/delete | GitHub/GitLab/Bitbucket/Azure DevOps/Gitea/Forgejo/Gogs/GitBucket **(8)** | `GET /repos/{owner}/{repo}/branches/{branch}` |
| F1-2 | `tag view` | list/create/delete | GitHub/GitLab/Gitea/Forgejo **(4)** | `GET /repos/{owner}/{repo}/tags/{tag}` |
| F1-3 | `deploy-key view` | list/create/delete | GitHub/GitLab/Bitbucket/Gitea/Forgejo/Gogs **(6)** | `GET /repos/{owner}/{repo}/keys/{id}` |
| F1-4 | `ssh-key view` | list/create/delete | GitHub/GitLab/Bitbucket/Gitea/Forgejo/Gogs **(6)** | `GET /user/keys/{id}` |
| F1-5 | `gpg-key view` | list/create/delete | GitHub/GitLab/Bitbucket/Gitea/Forgejo **(5)** | `GET /user/gpg_keys/{id}` |

### F2. create/delete はあるが edit がない

| # | コマンド | 現状 | API 対応サービス | 備考 |
|---|---|---|---|---|
| F2-1 | `webhook edit` | list/create/delete/test | GitHub/GitLab/Bitbucket/Backlog/Gitea/Forgejo/Gogs/GitBucket **(8)** | `PATCH /repos/{owner}/{repo}/hooks/{id}`。URL・イベント・シークレットの変更 |
| F2-2 | `org edit` | list/view/create/delete/members/repos | GitHub/GitLab/Gitea/Forgejo/Gogs **(5)** | `PATCH /orgs/{org}`。説明・表示名の変更 |
| F2-3 | `release asset edit` | list/upload/download/delete | GitHub/GitLab/Gitea/Forgejo **(4)** | `PATCH /repos/{owner}/{repo}/releases/{id}/assets/{id}`。アセット名の変更 |
| F2-4 | `tag-protect edit` | list/create/delete | GitLab/Gitea/Forgejo **(3)** | `PATCH /repos/{owner}/{repo}/tag_protections/{id}` |
...
| `status edit` | コミットステータスは追記モデル。新規作成で上書きする設計（GitHub/GitLab 共通） |
...
| `collaborator edit` | `collaborator add` を再実行すれば権限変更可能。専用 edit は不要 |
