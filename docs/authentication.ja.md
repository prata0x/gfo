...
**リトライ回数・タイムアウトは調整不可です。** HTTP クライアントはレートリミット（429）時に1回だけリトライし、リクエストタイムアウトは30秒（ストリーミングダウンロードは300秒）で固定されており、変更用の環境変数はありません。`--limit 0`（無制限取得）を大量データを持つページネーション対象に対して使うと、これらの固定値に先にぶつかりやすくなります。ホストが遅い・レートリミットが厳しい場合、待たされるのではなく失敗する点に注意してください。
