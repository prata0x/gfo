...
```
gfo init [--non-interactive] [--type TYPE] [--host HOST]
```
...
プロジェクト ID は `owner%2Frepo`（URL エンコード）で代用する。
...
| GitLab | ヘッダーベース | `X-Page` / `X-Total-Pages` header |
...
- **Issue / Repo / Release**: Gitea 互換 API で動作する（Gogs が Gitea の前身であり、エンドポイントパスが同一）
- **アダプター実装**: `GiteaAdapter` を継承し、PR 関連メソッドのみオーバーライドする
