...
```
gfo [--format table|json|plain] [--remote REMOTE] [--repo REPO] [--version] <command> <subcommand> [args]
```

| オプション | 値 | デフォルト | 説明 |
|-----------|---|-----------|------|
| `--format` | `table` \| `json` \| `plain` | `table` | 出力フォーマット。`config.toml` の `defaults.output` で変更可 |
| `--remote` | `REMOTE` | — | origin 以外の git remote を使用。指定リモートの URL で検出を行う。未指定時は `origin` を使用し、`origin` が存在しない場合は最初に見つかったリモートにフォールバックする |
| `--repo` | `REPO` | — | 対象リポジトリを直接指定。URL（HTTPS/SSH/SCP）または `HOST/OWNER/REPO` 形式を受け付ける。`--remote` と排他。指定時は git config ショートカットをスキップし、ホストから service_type を自動検出する |
...
| オプション | デフォルト | 説明 |
|-----------|-----------|------|
| `--title` | 現在のブランチの最後のコミットメッセージ subject | PR タイトル。Git リポジトリ外で省略した場合はエラー: `Error: --title is required when not in a git repository` |
| `--body` | 空文字列 | PR 本文 |
...
**ローカルブランチ命名規則**: ソースブランチ名をそのまま使用する（例: `feature/xxx`）。同名ブランチが既に存在する場合はエラー: `Error: Branch '{branch}' already exists. Delete it first or checkout manually.`
...
`--label` と `--assignee` は単一値のみ受け付ける（v1 スコープ）。複数ラベルでのフィルタが必要な場合は `--format json` + 外部ツール（`jq` 等）で対応する。データモデルの `assignees` は `list[str]` だが、CLI では単一値のみ受け付け、指定した値は要素1つのリストとしてアダプターに渡す。
...
**動作**: ホストの `service_type` に応じて HTTPS clone URL を直接構築し、`git clone` を実行する。API 経由でのリポジトリ情報取得は行わない。
...
非対応サービス（Bitbucket, Azure DevOps, Backlog, Gogs）は `NotSupportedError` を発生させる。
...
host = "github.com"        # gfo repo clone / gfo repo create でリポジトリ外から実行する際に使用
...
Windows 環境では、ファイル作成時に `icacls` で現在のユーザーのみにフルコントロール（`:F`）のアクセス権を付与する（ベストエフォート）。credentials.toml は読み書きが必要なため `:R`（読み取りのみ）ではなく `:F` を使用する。
...
5. **API エンドポイントプローブ**（未知ホストの場合）:
   ...
   | `GET /api/v4/version` | GitLab（注: 401/403 応答も GitLab と判定。下記注記参照） |
   | `GET /api/v3/` | 下記の判定ロジックで区別 |

   `GET /api/v3/` に応答した場合の判定ロジック:
   - レスポンス JSON に `"GitBucket"` 文字列が含まれる → `gitbucket`
   - レスポンスヘッダーに `X-GitHub-Enterprise-Version` がある → `github`（GHE）
   - 上記のいずれにも該当しない → `github`（GHE と仮定）

**注記**: GitLab セルフホスト版では `/api/v4/version` が認証を要求する場合がある。プローブが 401/403 を返した場合も GitLab と判定する（他サービスは同エンドポイントに 404 を返すため区別可能）。
...
| フィールド | 型 | 備考 |
|-----------|---|------|
| `number` | `int` | Azure DevOps の milestone（Iteration Path）と Backlog の milestone は v1 スコープでは未対応（`NotSupportedError`） |
| `title` | `str` | |
| `description` | `str \| None` | |
...
| `create_issue(title, body, assignee, label, **kwargs)` | Issue 作成。`**kwargs` で `type`（Azure DevOps の Work Item Type）と `priority`（Backlog の priorityId）を受け取る。他サービスでは無視する |
...
| `delete_release(tag)` | リリース削除。非対応サービスはデフォルト実装（`NotSupportedError`） |
...
| `delete_label(name)` | ラベル削除。非対応サービスはデフォルト実装（`NotSupportedError`） |
...
| `delete_milestone(number)` | マイルストーン削除。非対応サービスはデフォルト実装（`NotSupportedError`） |
...
| GitHub | o | o | o | o | o | o |
| GitLab | o | o | o | o | o | o |
| Bitbucket Cloud | o | o | o | x | x | x |
| Azure DevOps | o | o | o | x | o (*1) | o (*2) |
| Gitea | o | o | o | o | o | o |
| Forgejo | o | o | o | o | o | o |
| Gogs | x | o | o | o | x | x |
| GitBucket | o | o | o | o | o | o |
| Backlog | o (*3) | o | o | x | x | x |

*1: Azure DevOps の label は Work Item Tags として対応
*2: Azure DevOps の milestone は Iteration Path として対応（v1 スコープでは未実装、`NotSupportedError`）
*3: Backlog の PR merge は API なし（Web URL 案内）

非対応機能（x）は `NotSupportedError` を発生させ、§8.4 の動作に従う。

注: 上記マトリクスは各サービスの API ドキュメントに基づく最善の推定。実装時に検証して修正すること。

#### delete 対応状況

| サービス | release delete | label delete | milestone delete |
|---------|:-------------:|:------------:|:----------------:|
| GitHub | o | o | o |
| GitLab | o | o | o |
| Gitea | o | o | o |
| Forgejo | o（Gitea 継承） | o（Gitea 継承） | o（Gitea 継承） |
| GitBucket | o（GitHub 継承） | o（GitHub 継承） | o（GitHub 継承） |
| Bitbucket Cloud | x | x | x |
| Azure DevOps | x | x | x |
| Backlog | x | x | x |
| Gogs | x | x | x |
...
- `Retry-After` ヘッダーが存在しない場合は、デフォルトで 60 秒待機する
...
- `--format json` の場合もエラーは stderr に出力し、Web URL は stdout に出力する（フォーマットの影響を受けない）
...
| 例外 | 処理 |
|------|------|
| `requests.ConnectionError` | 接続エラー → `ConnectionError: Could not connect to {host}. Check your network connection.` |
| `requests.Timeout` | タイムアウト → `TimeoutError: Request to {host} timed out after {timeout}s.` |
...
|--------|------|
| 0 | 正常終了 |
| 1 | エラー（認証エラー、リソース未発見、非対応機能、ネットワークエラー等すべて） |
| 2 | コマンドライン引数エラー（argparse のデフォルト動作） |
...
| `exceptions.py` | カスタム例外定義（`NotSupportedError` 等） |
