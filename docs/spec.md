...
- **必要スコープ**: `read:repository:bitbucket`（リポジトリ読み取り）、`read:pullrequest:bitbucket` / `write:pullrequest:bitbucket`（PR 一覧・作成・マージ・クローズ）、`read:issue:bitbucket` / `write:issue:bitbucket`（Issue 一覧・作成・状態変更）
