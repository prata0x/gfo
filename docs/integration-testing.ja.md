...
4. Repository permissions:
   ...
   - Actions: **Read and write** （`ci trigger` / `ci retry` / `ci logs` テストに必要）
5. Account permissions:
   - Git SSH keys: **Read and write** （`ssh-key` テストに必要）
   - GPG keys: **Read and write** （`gpg-key` テストに必要）
6. Organization permissions（組織リポジトリの場合）:
   - Members: **Read** （`org members` テストに必要）
   - Administration: **Read and write** （`org create` / `org delete` テストに必要）
7. Generate token してコピー

> **注意**: `notification` テストには Fine-grained Token では対応できない。Classic Token の `notifications` スコープが必要。統合テストで `notification`、`pr checks`、`package` もテストする場合は Classic Token（スコープ: `repo`, `notifications`, `admin:public_key`, `read:org`, `admin:org`, `write:gpg_key`, `read:packages`, `write:packages`, `delete:packages`）を使用すること。

#### 環境変数

詳細は `github-spec.md §12` を参照。
...
> **Issue トラッカーの有効化**: Repository settings > Features > Issue tracker を有効にする（Issue テストに必要）。
>
> **Pipelines の有効化**: Repository settings > Pipelines > Settings > Enable Pipelines を有効にする（`gfo secret` / `gfo variable` テストに必要）。
...
4. **Scopes** で以下を選択:
   ...
   | `read:webhook:bitbucket` | Webhook 一覧取得 |
   | `write:webhook:bitbucket` | Webhook 作成・更新 |
   | `delete:webhook:bitbucket` | Webhook 削除 |
   | `read:ssh-key:bitbucket` | SSH 鍵・デプロイキー一覧取得 |
   | `write:ssh-key:bitbucket` | SSH 鍵・デプロイキー作成・更新 |
   | `delete:ssh-key:bitbucket` | SSH 鍵・デプロイキー削除 |
   | `read:user:bitbucket` | `get_current_user` に必要 |
   | `read:workspace:bitbucket` | `org list/view` に必要 |
   | `read:pipeline:bitbucket` | `secret/variable list`、`ci logs` に必要 |
   | `write:pipeline:bitbucket` | `secret/variable set`、`ci trigger` / `ci retry` に必要 |
   | `admin:pipeline:bitbucket` | `secret/variable` の作成・削除に必要 |
...
5. Scopes: **Custom defined** を選択
   ...
   - Build: **Read & execute** （`ci trigger` / `ci retry` テストに必要）
   - Project and Team: **Read** （`org list/view` に必要）
   - Member Entitlement Management: **Read** （`create_review`・`get_current_user` が使用する `/_apis/connectionData` エンドポイントに必要）
6. Create してトークンをコピー

> **注意**: `create_review` および `get_current_user` は `/_apis/connectionData` エンドポイントを使用する。Custom defined scopes で **Member Entitlement Management: Read** を含めるか、**Full access** の PAT を使用する必要がある。

#### 環境変数

詳細は `azure-devops-spec.md §12` を参照。
