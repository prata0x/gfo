...
> Milestone 削除の `{id}` はグローバル id（`iid` ではない）。詳細は「12. Milestone 仕様」を参照。
...
GitLab API はカラーコードを `#RRGGBB` 形式で返す。
gfo は先頭の `#` を除去して `RRGGBB` 形式で格納する。
...
`DELETE /projects/{id}/labels/{name}` でラベル名を URL エンコードして送信する。
