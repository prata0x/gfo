...
- 最新タグ: !`git describe --tags --abbrev=0 2>/dev/null || echo "(none)"`
- 最新タグ以降のコミット: !`git log --oneline $(git describe --tags --abbrev=0 2>/dev/null)..HEAD 2>/dev/null | head -30`
...
`src/gfo/__init__.py` の `__version__` と最新タグを比較する（タグの先頭 `v` を除去して比較）。

| 関係 | 意味 | 次の手順 |
|---|---|---|
| `__version__` == タグ | bump 必要（現コードは前リリースとバージョンが同じ） | 手順 2 へ |
| `__version__` > タグ | 既に main 上で bump 済み | 手順 6（タグのみ）へスキップ |
| `__version__` < タグ | 異常（作業ツリーが前リリースより古い） | 中断して報告 |
