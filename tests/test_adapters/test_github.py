...
# 共通 _to_* 変換（_to_pull_request / _to_issue / _to_repository / _to_release /
# _to_label / _to_milestone / _to_comment / _to_review / _to_branch / _to_tag /
# _to_commit_status / _to_webhook / _to_deploy_key）は GitHubLikeAdapter mixin に
# 単一実装され、GitHub/Gitea とも override しない。テストは tests/test_adapters/
# test_gitea.py が正本として網羅する（github_like.py を gitea のみで 100% カバー）。
# 重複を避けるため GitHub 側の同型 _to_* テストは持たない。
# GitHub 固有の created_at 欠落/空文字エッジは TestToIssueMissingCreatedAt を参照。
# _to_pipeline は GitHub 専用 override のためここでテストする。
...
class TestListPullRequests:
    ...
    def test_closed_excludes_merged(self, mock_responses, github_adapter):
        ...
        prs = github_adapter.list_pull_requests(state="closed")
        assert len(prs) == 1
        assert prs[0].number == 2

    def test_closed_fetches_all_pages_to_meet_limit(self, mock_responses, github_adapter):
        ...
        prs = github_adapter.list_pull_requests(state="closed", limit=2)
        # 全 4 件取得後、merged を除いた closed 2 件を返すべき
        assert len(prs) == 2
...
class TestCreateBranch:
    ...
    def test_create_from_short_sha(self, mock_responses, github_adapter):
        ...
        mock_responses.add(
            responses.POST,
            f"{REPOS}/git/refs",
            json={"ref": "refs/heads/new-branch", "object": {"sha": short_sha}},
            status=201,
        )
        mock_responses.add(
            responses.GET,
            f"{REPOS}/branches/new-branch",
            json=_branch_data(name="new-branch", sha=short_sha),
            status=200,
        )
