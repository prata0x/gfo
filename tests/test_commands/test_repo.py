...
class TestHandleLanguages:
    ...
    def test_non_json_formats(self, sample_config, capsys, fmt, expected):
        adapter = MagicMock()
        adapter.get_languages.return_value = {"Python": 45678, "Go": 12345}
        args = make_args()
        with patch("gfo.commands.repo.get_adapter", return_value=adapter):
            repo_cmd.handle_languages(args, fmt=fmt)
...
class TestHandleTopics:
    ...
    def test_jq_overrides_non_json_format(self, sample_config, capsys, fmt, action):
        adapter = MagicMock()
        method = {
            "list": "list_topics",
            "add": "add_topic",
        ...
        getattr(adapter, method).return_value = ["python", "cli"]
        args = make_args(
            topics_action=action,
            topic="cli",
            topics=["python", "cli"],
        )
        with patch("gfo.commands.repo.get_adapter", return_value=adapter):
